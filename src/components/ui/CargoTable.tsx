...
export function CargoTable({
    ...
    const [statusFilter, setStatusFilter] = useState<"all" | "active" | "pending" | "completed" | "transit" | "delivered" | "cancelled">("all");
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        {user?.role === 'driver' ? 'Manage and track your assigned cargo deliveries' : 'Track and manage all your cargo shipments'}
                    </p>
                </div>
                            ...
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-2xl font-bold text-gray-900">
                                        {cargos.filter(c => c.status === (user?.role === 'driver' ? "active" : "transit")).length}
                                    </p>
                                </div>
                        ...
                        <CardContent className="p-6">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-2xl font-bold text-gray-900">
                                        {cargos.filter(c => c.status === (user?.role === 'driver' ? "pending" : "delivered")).length}
                                    </p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <p className="text-2xl font-bold text-gray-900">
                                        {user?.role === 'driver'
                                            ...
                                            : formatFRW(cargos.reduce((sum, c) => sum + (c.cost || 0), 0))
                                        }
                                    </p>
                                </div>
                            ...
                            <Table className="w-ful-l">
                                <TableHeader>
                                    <TableRow>
                                        ...
                                        <TableHead className="text-xs font-medium text-gray-600 w-24">Cargo ID</TableHead>
                                        {user?.role === 'driver' ? (
                    ...
                    {showPagination && totalPages > 1 && (
                        <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100">
                            <div className="text-sm text-gray-500">
                                Showing {startIndex + 1} to {Math.min(endIndex, filteredCargos.length)} of {filteredCargos.length} results
                            </div>
                            <div className="flex items-center gap-2">
                                <Button
                                    variant="outline"
            ...
            <div className="md:hidden">
                ...
                {showPagination && totalPages > 1 && (
                    <div className="flex items-center justify-between mt-6">
                        <div className="text-sm text-gray-500">
                            {startIndex + 1}-{Math.min(endIndex, filteredCargos.length)} of {filteredCargos.length}
                        </div>
                        <div className="flex items-center gap-2">
                            <Button
                                variant="outline"
