...
class GpsTrackingService {
  ...
  async requestPermission(): Promise<boolean> {
    ...
    try {
      if (Capacitor.isNativePlatform()) {
        // Request permissions using Capacitor
  ...
  private convertPositionToLocationData(
    position: GeolocationPosition | any
  ): LocationData {
    return {
  ...
  async shareLocationWithAdmin(
    ...
    try {
      // Store location locally for offline sync
      await offlineStorageService.addToSyncQueue({
        entityType: "gps_location",
        action: "create",
        payload: {
          driverId,
          cargoId,
      ...
      // Try to upload immediately if online using axios
      const response = await axiosInstance.post("/driver/location", {
        driverId,
        cargoId,
        location: {
          latitude: location.latitude,
  ...
  async batchUploadLocations(
    ...
    try {
      const response = await axiosInstance.post("/batch/gps-locations", {
        locations,
      });
